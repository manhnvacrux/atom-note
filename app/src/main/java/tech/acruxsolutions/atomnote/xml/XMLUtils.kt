    ...
    companion object {
        ...
        fun readBackupFromFile(inputStream: InputStream): Backup {
            ...
            while (parser.next() != XmlPullParser.END_DOCUMENT) {
                if (parser.eventType == XmlPullParser.START_TAG) {
                    when (parser.name) {
                        XMLTags.Notes -> baseNotes =
                            parseList(
                                parser,
                                XMLTags.Notes
                            )
                        XMLTags.DeletedNotes -> deletedNotes =
                            parseList(
                                parser,
                                XMLTags.DeletedNotes
                            )
                        XMLTags.ArchivedNotes -> archivedNotes =
                            parseList(
                                parser,
                                XMLTags.ArchivedNotes
                            )
        ...
        fun writeBackupToFile(backup: Backup, file: File) {
            ...
            appendBackupList(
                XMLTags.ArchivedNotes,
                xmlSerializer,
                backup.archivedBaseNotes
            )
            appendBackupList(
                XMLTags.DeletedNotes,
                xmlSerializer,
                backup.deletedBaseNotes
            )
            ...
            while (parser.next() != XmlPullParser.END_DOCUMENT) {
                if (parser.eventType == XmlPullParser.START_TAG) {
                    when (parser.name) {
                        ...
                        XMLTags.Label -> labels.add(parser.nextText())
                        XMLTags.ListItem -> listItems.add(
                            parseItem(
                                parser
                            )
                        )
                        XMLTags.Span -> spans.add(
                            parseSpan(
                                parser
                            )
                        )
