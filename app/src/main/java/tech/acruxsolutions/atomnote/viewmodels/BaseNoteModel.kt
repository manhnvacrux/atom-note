...
class BaseNoteModel(app: Application) : AndroidViewModel(app) {
    ...
    private val observer: AtomNoteFileObserver
    private val deletedObserver: AtomNoteFileObserver
    private val archivedObserver: AtomNoteFileObserver

    init {
        observer =
            AtomNoteFileObserver.getInstance(
                notesHelper.getNotePath()
            )
        deletedObserver =
            AtomNoteFileObserver.getInstance(
                notesHelper.getDeletedPath()
            )
        archivedObserver =
            AtomNoteFileObserver.getInstance(
                notesHelper.getArchivedPath()
            )
    ...
    class AtomNoteFileObserver : FileObserver {
        ...
        private constructor(filePath: String) : super(filePath,
            mask
        )
        ...
        companion object {
            ...
            fun getInstance(file: File) : AtomNoteFileObserver {
                return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    AtomNoteFileObserver(
                        file
                    )
                } else AtomNoteFileObserver(
                    file.path
                )
            }
        }
